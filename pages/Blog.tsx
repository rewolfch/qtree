...
const Blog: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-24">
      <SEO 
        title="Blog & Insights" 
        description="Neueste Artikel zu Quality Engineering, Leadership und AI-gestütztem Testen. Insights aus der Praxis von Serge Baumberger."
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h1 className="text-5xl md:text-6xl font-black text-slate-900 mb-6 tracking-tight">Insights & Strategy.</h1>
          <p className="text-xl text-slate-600 leading-relaxed">
            Aktuelle Gedanken zu Quality Engineering, Leadership und Erfahrungen aus der Praxis direkt vom Autor des Quality Tree Frameworks.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-20 relative group">
           ...
           <input 
            type="text" 
            placeholder="Nach Artikeln suchen (z.B. Testing, AI, Leadership...)"
            value={searchQuery}
           ...
           {searchQuery && (
             <div className="absolute right-6 top-1/2 -translate-y-1/2 bg-slate-100 text-slate-500 px-3 py-1 rounded-lg text-xs font-bold animate-fade-in">
               {filteredPosts.length} Ergebnisse
             </div>
           )}
        </div>
            ...
            {filteredPosts.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                {filteredPosts.map(post => (
                  <article key={post.id} className="flex flex-col bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group h-full">
                    ...
                    <div className="p-10 flex-grow flex flex-col">
                      ...
                      <div className="mt-auto pt-8 border-t border-slate-50 group-hover:border-slate-100">
                        <Link to={`/blog/${post.id}`} className="inline-flex items-center text-slate-900 font-black text-xs uppercase tracking-widest hover:text-brand-600 transition-all gap-2 group/btn">
                          Beitrag lesen <ArrowRight className="h-4 w-4 group-hover/btn:translate-x-2 transition-transform" />
                        </Link>
                      </div>
                    </div>
                  </article>
              ...
              <div className="text-center py-24 bg-white rounded-[3rem] border border-slate-100">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2">Keine Artikel gefunden</h3>
