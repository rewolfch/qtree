...
const Framework: React.FC = () => {
  ...
  const filteredBranches = branches.filter(branch => 
    ...
    branch.levels.some(l => l.title.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-24">
      <SEO 
        ...
        description="Erkunden Sie die 8 Dimensionen und 90 Praktiken des Quality Tree Frameworks. Von Konfigurationsmanagement bis zu AI-gestütztem Testing."
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-2xl mx-auto mb-20 relative group">
           ...
           <input 
            type="text" 
            placeholder="Nach Themen suchen (z.B. CI/CD, Automatisierung, AI...)"
            value={searchQuery}
           ...
           {searchQuery && (
             <div className="absolute right-6 top-1/2 -translate-y-1/2 bg-slate-100 text-slate-500 px-3 py-1 rounded-lg text-xs font-bold animate-fade-in">
               {filteredBranches.length} Ergebnisse
             </div>
           )}
        </div>
        ...
        {filteredBranches.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredBranches.map((branch) => {
              ...
              return (
                ...
                >
                  <div className={`absolute -right-6 -top-6 w-40 h-40 opacity-[0.03] group-hover:opacity-[0.1] transition-opacity duration-700 ${visual.color} pointer-events-none`}>
                  ...
                  <div className="flex items-center text-brand-600 font-black text-[10px] uppercase tracking-widest mt-auto pt-8 border-t border-slate-50">
                    Levels entdecken <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-2 transition-transform" />
                  </div>
                </Link>
              );
            })}
          ...
          <div className="text-center py-24 bg-white rounded-[3rem] border border-slate-100">
             ...
             <h3 className="text-xl font-bold text-slate-900 mb-2">Keine Äste gefunden</h3>
             <p className="text-slate-500">Versuchen Sie es mit einem anderen Suchbegriff.</p>
             <button onClick={() => setSearchQuery("")} className="mt-6 text-brand-600 font-bold hover:underline">Suche zurücksetzen</button>
          </div>
        )}
        
        {/* Footer CTA */}
        <div className="mt-32 p-16 bg-slate-900 rounded-[4rem] text-center text-white relative overflow-hidden">
           <div className="absolute inset-0 bg-brand-600 opacity-5 blur-[120px]"></div>
           <div className="relative z-10 max-w-3xl mx-auto">
             ...
             <h3 className="text-4xl font-black mb-8">Bereit, Ihren Baum zu pflanzen?</h3>
             <p className="text-slate-400 text-lg mb-12 leading-relaxed">Nutzen Sie das Assessment-Tool, um Ihren IST-Zustand zu visualisieren und eine klare Roadmap für die nächsten 12 Monate zu erhalten.</p>
             <Link to="/app" className="inline-flex items-center bg-brand-600 hover:bg-brand-500 text-white px-12 py-5 rounded-2xl font-black text-xl transition-all shadow-2xl hover:-translate-y-1">
                Zum Assessment Tool <ArrowRight className="ml-3 h-6 w-6" />
             </Link>
           </div>
        </div>
      </div>
